...
for p in sorted(SUITE.glob("lp_*.mat")):
    ...
    try:
        red = presolve_matrix(
            from_scipy_sparse(A), b.tolist(), c.tolist(), lo.tolist(), hi.tolist(), algorithm="auto"
        )
        if red is None:
            continue
        M = red._matrix
    ...
    if (
        ...
        and pr <= 4000
        and pc <= 30000
        and _ipm_stall_risk(list(red.c), list(red.lo), list(red.hi), pnnz, pc)
    ):
        route = "SIMPLEX"
...
for r in rows:
    print(f"{r[0]:16s} {r[1]:>6} {r[2]:>6} {r[3]:>7} {r[4]:>6.2f}  {r[5]}")
